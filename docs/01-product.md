...
## Concept principal
Bizko est un outil utilitaire (pas une plateforme au MVP) qui permet à un indépendant service de créer un profil public unique `bizko.co/username` qui convertit.
...
## Profil public
URL : `bizko.co/{username}` (ex: `bizko.co/aminata_photo`)
...
- Dans le dashboard : bouton "Générer mon QR" → affiche QR de `bizko.co/username` (lib qrcode) → Télécharger PNG 800x800 + Partager.
...
- URL publique : `bizko.co/{username}` (pas de sous-domaine au MVP).
