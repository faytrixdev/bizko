...
Dashboard / Éditeur
  ↓ [Voir mon profil]
Profil public bizko.co/{username}
```

Points clés : pas de friction email verification bloquante au MVP. Username validé avant suite.
...
```
Lien bizko.co/{username} (reçu sur WhatsApp/Insta/QR)
  ↓
...
Dashboard → [Générer mon QR]
  ↓
Modal : QR de bizko.co/{username} (qrcode lib)
...
```
bizko.co/login → email+password → Dashboard
