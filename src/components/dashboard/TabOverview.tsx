...
export function TabOverview({ publicUrl, username, analytics }: TabOverviewProps) {
  const { t } = useI18n();
      ...
      <div className="border border-gray-100 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow duration-300">
        ...
        {analytics.status === "error" && (
          <div className="rounded-2xl border border-red-200 bg-red-50 p-6 text-center">
            <p className="text-sm text-red-700">Impossible de charger tes statistiques. Vérifie ta connexion puis réessaie.</p>
          </div>
        )}

        {analytics.status === "success" && <OverviewContent analytics={analytics} />}
      </div>
    </div>
  );
}

function OverviewContent({ analytics }: { analytics: Extract<DashboardAnalytics, { status: "success" }> }) {
  const { t } = useI18n();
  const { views, waClicks, daily, breakdown, views7d, clicks7d } = analytics;
