...
function ProfileContent({
  ...
  return (
    <>
      ...
      {/* Bio */}
      {variant !== "compact" && (
        <p className="text-xs text-gray-500 leading-5 mb-4 line-clamp-2">
          {bio}
        </p>
      )}
        ...
        {services.map((s, i) => (
          ...
          >
            <div className="min-w-0">
              ...
              <div className="text-[11px] font-bold text-[#FF6B35] mt-0.5">
                {s.price}
              </div>
            </div>
      ...
      {/* Portfolio */}
      {variant === "detailed" && portfolio && portfolio.length > 0 && (
        <div className="mt-4">
          <div className="grid grid-cols-3 gap-1.5">
            {portfolio.map((p, i) => (
              <div
                key={i}
                className={`aspect-square rounded-lg bg-gradient-to-br ${portfolioGradients[i % portfolioGradients.length]} flex items-center justify-center`}
              >
                {p.label && (
  ...
  if (frame) {
    return (
      <div className="relative mx-auto">
        {/* Ambient glow */}
        <div className="absolute -inset-4 bg-gradient-to-b from-[#FF6B35]/8 via-transparent to-transparent rounded-[2.5rem] blur-2xl pointer-events-none" />
